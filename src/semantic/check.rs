...
use crate::{
    ast::{Block, Decl, SpannedNode, Stmt},
    semantic::{
        ...
        match self.deref() {
            Var { ident, ty, init } => {
                if let Some(init) = init {
                    ...
                    env.bind(&ident.0, ty.deref().into(), self.id);
                }
            }
            Func { ident, sig, body } => {
                env.bind(&ident.0, sig.deref().into(), self.id);
                ...
                sig.params
                    .iter()
                    .for_each(|p| env.bind(&p.ident.0, p.ty.deref().into(), self.id));
